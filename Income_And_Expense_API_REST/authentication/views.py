...
class RegisterView(generics.GenericAPIView):
    ...
    renderer_classes = (UserRenderer,)  # renderer trả về json

    def post(self, request):
        user = request.data
        serializer = self.serializer_class(data=user)
        serializer.is_valid(raise_exception=True)  # check if any exception
        serializer.save()
        ...
        abs_url = 'http://' + current_site + \
            relative_link + "?token=" + str(token)  # current site = host
        email_body = 'Hi ' + user.username + \
                     ' Use the link below to verify your email \n' + abs_url  # message to send
        data = {'email_body': email_body, 'to_email': user.email,
                'email_subject': 'Verify your email'}

        Util.send_email(data)  # send email to verify
